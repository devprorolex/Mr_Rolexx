...
from NOVA_MUSIC import YouTube, app
from NOVA_MUSIC.core.call import NOVA
from NOVA_MUSIC.misc import SUDOERS, db
from NOVA_MUSIC.utils import bot_sys_stats
from NOVA_MUSIC.utils.database import (
    get_active_chats,
    get_lang,
    is_active_chat,
    is_music_playing,
...
from NOVA_MUSIC.utils.formatters import seconds_to_min
from NOVA_MUSIC.utils.inline import (
    stream_markup,
    stream_markup_timer,
    telegram_markup,
...
from NOVA_MUSIC.utils.stream.autoclear import auto_clean
from NOVA_MUSIC.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await NOVA.pause_stream(chat_id)
        ...
        await NOVA.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await NOVA.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await NOVA.skip_stream(chat_id, link, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await NOVA.skip_stream(chat_id, queued, video=status, image=image)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
